...
public class AccountController extends BaseController {  
        ...
        if(account==null || account.getEnable()==false || account.getRole()==null){
        	...
        	for(Authority authority : roleAuthorities){ 	  		
        		...
        		if(parentAuthorities.size()>=2){
        			PermissionMenu permissionMenu=new PermissionMenu(parentAuthorities.get(parentAuthorities.size()-1).getId(),parentAuthorities.get(parentAuthorities.size()-1).getName(),parentAuthorities.get(parentAuthorities.size()-2).getId(),parentAuthorities.get(parentAuthorities.size()-2).getName(),authority.getName(),authority.getMatchUrl());
        			permissionMenus.add(permissionMenu);
        		}else if(parentAuthorities.size()==1){
        			PermissionMenu permissionMenu=new PermissionMenu(parentAuthorities.get(0).getId(),parentAuthorities.get(0).getName(),authority.getId(),authority.getName(),authority.getName(),authority.getMatchUrl()); 
        			permissionMenus.add(permissionMenu);
        		}else{
    ...
    public String list(HttpServletRequest request, Model model, AccountSearchModel searchModel){ 			
        ...
        System.out.println("765757575=="+request.getServletPath());
	...
	public String delete(HttpServletRequest request,@PathVariable(value="id") Integer id) throws ValidatException, EntityOperateException{
	...
	public String enable(HttpServletRequest request,@PathVariable(value="id") Integer id) throws ValidatException,EntityOperateException{
		Account account=accountService.get(id);
	...
	public String disable(HttpServletRequest request,@PathVariable(value="id") Integer id) throws ValidatException,EntityOperateException{
	  Account account= accountService.get(id);
	...
	public String forget(HttpServletRequest request,ModelMap modelMap,@RequestParam("username") String username) throws ValidatException,EntityOperateException, Exception{
		if (username!=null &&!"".equals(username)) {	
		  Account account=accountService.findAccountByName(username.trim());
	      String email =account.getEmail();
	      System.out.println("9999=="+email);
	      String pwd=account.getPassword();
	      System.out.println("22222=="+pwd);
	      EmailUtils.sendMessage(email, "原始密码", "尊敬的用户，您的密码为:"+pwd+"请注意保管，防止被盗！");
	      modelMap.put("message", "邮件已发送至您的邮箱，请注意查收！");
		}
